...
public record ChannelCredentialCreateCommand(
    ...
    public ChannelCredentialCreateCommand {
        if (name == null || name.strip().length() < 2 || name.strip().length() > 64) {
            throw new IllegalArgumentException("name 长度必须为 2—64");
        }
        if (!ChannelCredentialListItem.SOURCE_INLINE.equals(secretSource)
                && !ChannelCredentialListItem.SOURCE_EXTERNAL.equals(secretSource)) {
        ...
        if (ChannelCredentialListItem.SOURCE_EXTERNAL.equals(secretSource)
