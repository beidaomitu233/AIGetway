 ...
 * model_id 或 upstream_model_id 必须解析到同一 Provider；
 * channel_credential_id 可空（由目标池选择）；不接收真实 Secret。
 */
@JsonInclude(JsonInclude.Include.NON_NULL)
public record ChannelCheckCommand(
        String modelId,
    ...
    public boolean hasModelTarget() {
        return (modelId != null && !modelId.isBlank()) || (upstreamModelId != null && !upstreamModelId.isBlank());
    }
