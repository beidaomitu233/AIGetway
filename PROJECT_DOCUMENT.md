...
最终生产验收还要求真实 PostgreSQL/MySQL 5.7/8.0 的版本化数据库升级、失败恢复与仓储矩阵，物理双实例与 Redis 故障恢复，以及 200 条业务 HTTP 流、10 分钟稳态和 WebFlux 管理 API/Admin UI 端到端验证。Java 17/21 × Spring Boot 3.3.8/3.4.13/3.5.5 的 14 模块构建矩阵已通过，Reactive Standalone Client、宿主端口 Embedded Chat、无 Web 空快照，以及 H2 管理 JDBC 存储与运行端口自动接线已验证；这些证据不能替代尚未执行的外部环境门禁。
