...
需求基线为仓库根目录《轻享AI-企业AI中台-产品需求说明书-PRD-V2.0.md》。唯一集成与领取起点为远程 `origin/dev`；已纳入 V2 代码基点 `9e074b5`、纯 V2 文档 `c69af23` 和前端阻塞登记 `7669954`。用户已确认 V2 作为主线并要求开始前后端开发。旧 fix 分支不再作为开发起点。

任务领取与文件占用见 [TASK_STATUS.md](TASK_STATUS.md)，契约差异、开发假设和环境绕行方法见 [COMMUNICATION.md](COMMUNICATION.md) 第 6 节；BE-P20 最新架构处理与验收缺口见第 8 节及前后端/数据库 Plan 的同名处理结论。FE-P20、BE-P20 已被领取，BE-P20 仍阻塞并保留负责人及占用；已确认子项可由原负责人继续，未确认选项只约束对应子项。真实 Provider、企业身份及首调 E2E 未验收，方案确认不等于业务交付。
...
| 虚拟模型与路由 | 旧 Alias/候选资产、应用模型约束 | 需收口 V2 术语、能力交集和应用影响 | FE-P21、BE-P21、DB-P21 |
