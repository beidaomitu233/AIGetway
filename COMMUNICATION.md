...
| 编号 | 级别 | 问题与依据 | 涉及文件/接口/表 | 根因与修复 | 验证结果 | 负责人 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| UI-ANT-342-001 | P1 | 迁移后的页面仍通过旧的纯文本 StatusText 输出状态，颜色和状态语义没有统一进入 Ant Design 组件体系；渠道、治理和观测列表均可见。 | `light-ai-admin-ui/src/components/StatusText.vue`；所有调用方数据契约不变 | 使用 Ant Design `Tag` 输出状态，按成功、处理中、错误和默认状态映射颜色，保留 `.lai-status-text`、占位文本和显示标签。 | `npm run typecheck` 通过；渠道详情、治理熔断、调用记录回归 24 项通过。 | 代码审查与修复模型/codex-ui-cleanup-0913 | 待复验 |
| UI-ANT-342-002 | P1 | 凭证详情与访问详情仍残留原生卡片容器，核心敏感信息区未进入 Ant Card 体系；进入访问凭证或池详情即可复现。 | `light-ai-admin-ui/src/pages/access/AccessDetailPage.vue`、`light-ai-admin-ui/src/pages/credentialPools/PoolDetailPage.vue`；敏感字段裁剪与凭证 API 契约不变 | 将详情区块容器迁移为 Ant Design `Card`，保留敏感字段裁剪、一次显示、错误状态和旧入口重定向。 | `npm run typecheck` 通过；`poolPages.test.ts` 5 项通过。 | 代码审查与修复模型/codex-ui-cleanup-0913 | 待复验 |
| UI-ANT-342-003 | P1 | 发布记录、运行配置、额度流水、草稿发布和应用表单仍存在旧卡片容器，视觉体系未完全统一；对应低频页面可见。 | `light-ai-admin-ui/src/pages/config/PublishRecordDetailPage.vue`、`RuntimeConfigPage.vue`、`DraftsPage.vue`、`PublishPage.vue`、`UsageAdjustmentsPage.vue`、`ApplicationFormPage.vue` | 将可安全迁移的区块改为 Ant Design `Card`，保留条件渲染、配置权限、版本冲突和表单行为。 | `npm run typecheck` 通过；配置发布、运行配置、用量和应用回归 57 项通过。 | 代码审查与修复模型/codex-ui-cleanup-0913 | 待复验 |
| UI-ANT-342-004 | P1 | 渠道详情和调用详情仍保留旧容器，健康检测、凭证和 Attempt 信息的视觉边界不统一；对应详情页可见。 | `light-ai-admin-ui/src/pages/providers/ProviderDetailPage.vue`、`light-ai-admin-ui/src/pages/traces/TraceDetailPage.vue` | 将详情区块迁移为 Ant Design `Card`，保留检测、凭证脱敏、Attempt 展开和导出行为。 | `npm run typecheck` 通过；渠道和调用记录回归 27 项通过。 | 代码审查与修复模型/codex-ui-cleanup-0913 | 待复验 |
## UI-R343 Ant Design 最终集成审查（2026-09-13，codex-ui-final-review-0913）

| 编号 | 级别 | 问题与依据 | 涉及文件/接口/表 | 根因与修复 | 验证结果 | 负责人 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| UI-ANT-343-001 | P1 | 已迁移页面和共享视觉已通过全量测试，但部分低频页面仍保留原生 `.lai-card` 结构，尚未全部替换为 Ant Card；代码审查可见，不能宣称全后台从零完成。 | `light-ai-admin-ui/src/pages/**` 其余旧卡片页面；业务 API 契约不变 | 已完成核心路径 Ant Card/Tag/Progress 迁移和全局 token；剩余页面需继续按 UI-R342 清理波次迁移，不能通过隐藏旧结构规避。 | 全量回归 33 个测试文件、255 项通过；生产构建通过；低频页面视觉尚未逐页浏览器复验。 | 代码审查与修复模型/codex-ui-final-review-0913 | 待修复 |
