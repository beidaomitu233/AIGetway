...
| C-022 | 前端执行模型 | 契约补充 | bootstrap.roles[]/permissions[]取值口径未冻结，导航与动作守卫需要稳定标识 | 前端按PROJECT_DOCUMENT 2.4.2矩阵推导：角色码SYSTEM_ADMIN/OPERATOR/DEVELOPER/VIEWER，权限键如provider.view/provider.manage/provider.check（见light-ai-admin-ui/src/app/permissions.ts） | FE-002、src/app/permissions.ts、navConfig.ts | BE-002 | 无 | 待确认 | 后端冻结bootstrap枚举后同步前端常量与四角色夹具；前端不新增权限判定逻辑 |
...
| FE-H01 | 前端执行模型 | Git | FE-P01基础包FE-001~006完成 | 工程light-ai-admin-ui（Vue3+TS+Vite+Router+Pinia，依赖精确锁定）；请求层/守卫/列表URL筛选/表单与危险操作/敏感信息与嵌入隔离组件；空根与/light-ai双根深链刷新验证通过 | light-ai-admin-ui全部 | 无（BE-002未交付，bootstrap使用mocks/契约夹具） | 无 | 执行中 | 分支feature/frontend-foundation；commit见本包feat(frontend)提交；npm run lint、typecheck、test（53例通过）、build全部通过；待审查合入dev后由后端联调替换mock |
