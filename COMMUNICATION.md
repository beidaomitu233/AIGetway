...
验证证据：当前修复在 Java 17 完整 `mvn verify` 成功，14 个 Reactor 模块、408 项总计，403 项执行、5 个外部数据库用例跳过，0 failure、0 error；Redis 11 项实测包含两个独立连接。Java 21 上一基线完整验证为 404 项总计、399 项执行、5 项跳过，0 failure、0 error。Java 17/21 × Boot 3.3.8/3.4.13/3.5.5 全模块打包通过。前端 lint 0 error/37 warning，typecheck、160 例测试及 build 通过。可执行 JAR 启动后健康、UI 与未认证边界响应符合预期；既有完整 H2 + Stub Provider 冒烟继续覆盖配置创建、发布、同步 Chat 与两类 SSE。Starter 新增 Servlet Embedded JDBC、Reactive Standalone、Reactive Embedded 实际 Chat 及无 Web/无存储空快照装配测试。

生产验收仍由 CR-015、CR-016 的外部环境门禁阻塞。版本化迁移能力已交付，但需真实 PostgreSQL、MySQL 5.7/8.0 执行升级、失败恢复和回滚；性能兼容需补 WebFlux 管理 API/Admin UI 与原生存储端到端、物理双实例、200 条业务 HTTP 流、2 分钟预热、10 分钟稳态及真实 DB/Redis 故障演练。当前允许单实例 Standalone、Servlet Embedded，以及宿主提供运行端口的 Reactive Embedded 功能验收；不建议签署最终生产验收或合并到 main 发布。
