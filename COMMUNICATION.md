...
| H-008 | 后端执行模型（会话B） | Git/任务领取 | 领取后端接入对象包BE-P03全部6项（BE-013 Credential查询写入轮换检测、BE-014 Model字段与能力管理、BE-015 模型导入与批量检测、BE-016 Alias列表详情写入删除、BE-017 候选增改删除探测、BE-018 候选原子重排），领取即锁定，其他协作者请勿重复领取；BE-P02已由H-007并行领取，两包文件集不重叠（本包：credential/credential_secret/provider_model/model_alias/route_candidate/batch_check仓储与服务，client access DTO；对方：provider/credential_pool/检测编排），storage check仓储与ImpactAnalysis可能双写，合并时按先落地者保留单一实现 | 分支feature/backend-models（git worktree C:\AIgetway\AIGetway-models，基于dev f6fc471）；锁定声明写入BACKEND_PLAN.md BE-P03包头；与feature/backend-provider并行执行，合并顺序后至者rebase；交付后在本行补登commit、测试命令与结果 | light-ai-client（access DTO）、light-ai-storage-jdbc（credential/model/alias/check仓储）、light-ai-admin（credential/model/alias服务与控制器） | BE-013~BE-018 | credential、credential_secret、provider_model、model_alias、route_candidate、batch_check_job/item、provider_check_record | 已完成 | 2026-09-05交付：分支feature/backend-models，commit见本包feat(backend)提交；测试命令mvn -B test（Maven 3.9.9，Java 18编译--release 17），全工程121例全部通过0失败（本包新增：client access DTO序列化与SecretSource/CheckMode校验、SecretManager加密往返/二次确认/掩码、CredentialService草稿事务+同池唯一+轮换secret_version+删除CAPACITY_IN_USE+审计关联、ImpactService摘要稳定与IMPACT_ANALYSIS_EXPIRED、ModelAlias别名不可变+重复拒绝、RouteCandidate同Provider+重复三元组+原子重排版本冲突整批回滚+weight保持、ProviderModel能力完整性C-014+字段不可变+逐对象导入C-005、BatchCheck同Provider+超限拒绝+取消JOB_ALREADY_FINISHED+单项隔离+明细快照遍历修复ConcurrentModificationException）。遗留：真实PostgreSQL约束/行锁/同事务原子性待DB-P02迁移落地后联调复核；available-models与批量检测执行依赖BE-P05 Adapter注入（CheckInvoker/ModelListPort端口已定义，桩验证编排）；探测CAPACITY_LIMITED与容量预占依赖BE-P04；client.access.ImpactAnalysis若与BE-P02分支重复，合并时保留单一实现。提醒：并行会话同包双写仍禁止，合并顺序后至者rebase |
