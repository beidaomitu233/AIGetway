...
<template>
  <div class="lai-dialog-overlay">
    <div class="lai-dialog" role="dialog" aria-modal="true" aria-labelledby="application-secret-title">
      <h2 id="application-secret-title" class="lai-dialog-title">请立即保存应用密钥</h2>
      ...
      <div class="lai-dialog-actions">
        <button type="button" class="lai-btn" :disabled="copying" @click="copy">{{ copied ? '已复制' : '复制密钥' }}</button>
        <button type="button" class="lai-btn lai-btn-primary" @click="$emit('close')">我已保存</button>
      </div>
    </div>
  </div>
</template>
