...
import { fetchAvailableModels, listProviderModels, type ProviderModelImportCandidate } from '@/api/providerModels'
...
const catalogModels = ref<ProviderModelImportCandidate[]>([])
...
async function loadCatalog(): Promise<void> {
  ...
  try {
    const models = await fetchAvailableModels(providerId.value, {
      source: 'PROVIDER_API',
...
watch(providerId, () => { detail.value = null; checkOpen.value = false; catalogModels.value = []; catalogKeyword.value = ''; catalogLoaded.value = false; catalogError.value = null; void load() })
      ...
      <Card :bordered="false" class="lai-card">
        <div class="card-heading">
          <div>
            ...
            <p class="lai-card-hint">
              直接从渠道查询可用模型；查询结果仅用于映射选择与检测，不会自动写入全局模型记录。
            </p>
          </div>
          <Button
            type="primary"
        ...
        >
          <li
            ...
            :key="model.model_id"
          >
            <span class="lai-related-name">{{ model.display_name || model.model_id }}</span>
            <span class="lai-related-meta">
              {{ model.model_id }}
              <template v-if="model.max_output_tokens"> · 最大输出 {{ model.max_output_tokens.toLocaleString() }}</template>
              <template v-if="model.support_stream !== null"> · {{ model.support_stream ? '支持流式' : '不支持流式' }}</template>
            </span>
          </li>
        </ul>
        <p
