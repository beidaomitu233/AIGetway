...
export function fetchProviderModel(id: string, signal?: AbortSignal): Promise<ProviderModelDetail> {
  return request<ProviderModelDetail>({ path: `/upstream-models/${id}`, signal })
}

export function createProviderModel(command: ProviderModelCommand): Promise<ManagementOperationResult> {
  return request<ManagementOperationResult>({ path: '/upstream-models', method: 'POST', body: command })
}
...
export function updateProviderModel(
  ...
  return request<ManagementOperationResult>({ path: `/upstream-models/${id}`, method: 'PUT', body: command })
}

export function fetchAvailableModels(
  providerId: string,
  query: { source: string; channel_credential_id?: string; keyword?: string | undefined },
  signal?: AbortSignal,
): Promise<ProviderModelImportCandidate[]> {
  return request<ProviderModelImportCandidate[]>({
    path: `/channels/${providerId}/available-models`,
    query,
    signal,
  })
}
...
export function importProviderModels(command: ProviderModelImportCommand): Promise<ImportResult> {
  return request<ImportResult>({ path: '/upstream-models/import', method: 'POST', body: command })
}
...
export function startBatchCheck(command: {
  ...
  return request<BatchCheckJob>({ path: '/upstream-models/batch-check', method: 'POST', body: command })
}

export function fetchBatchCheckJob(id: string, signal?: AbortSignal): Promise<BatchCheckJobDetail> {
  return request<BatchCheckJob & { items: BatchCheckItem[] }>({ path: `/batch-check-jobs/${id}`, signal }).then((payload) => ({ job: payload, items: payload.items }))
}
...
export function enableProviderModel(id: string, version: number): Promise<ManagementOperationResult> {
  return request<ManagementOperationResult>({ path: `/upstream-models/${id}/enable`, method: 'POST', body: { version } })
}

export function disableProviderModel(id: string, version: number): Promise<ManagementOperationResult> {
  return request<ManagementOperationResult>({ path: `/upstream-models/${id}/disable`, method: 'POST', body: { version } })
}

export function deleteProviderModel(id: string, version: number): Promise<ManagementOperationResult> {
  return request<ManagementOperationResult>({ path: `/upstream-models/${id}`, method: 'DELETE', body: { version } })
}
...
export async function fetchProviderCredentials(
  ...
  const result = await request<PageResult<{ id: string; name: string; enabled: boolean }>>({
    path: `/channels/${providerId}/credentials`,
    query: { enabled: true, page: 1, page_size: 100, sort: 'name' }, signal,
  })
