...
describe('ProviderDetailPage（FE-009/FE-010）', () => {
  ...
  it('按关键词查询渠道实时模型目录', async () => {
    stub = installJsonFetchStub((context) => {
      const base = baseHandler()(context)
      if (base) return base
      const { url, method } = context
      if (method === 'GET' && url.pathname.endsWith('/admin/channels/prov-1/available-models')) {
        return dataEnvelope([
          {
            model_id: 'qwen-max',
    ...
    const catalogCall = stub.calls.find((call) => call.url.includes('/available-models'))
    expect(catalogCall?.url).toContain('source=PROVIDER_API')
    expect(catalogCall?.url).toContain('keyword=qwen')
  })
