...
describe('FE-213 未知能力与权限', () => {
  it('禁用草稿保留未知能力和空价格，不伪造支持或免费', async () => {
    ...
    expect(JSON.parse(String(post![1]!.body))).toMatchObject({ enabled: false, support_stream: null, support_temperature: null, input_price: null, output_price: null })
    wrapper.unmount()
  })
  it('权限撤销后隐藏保存且直接提交不发请求', async () => {
    const fetchMock = stubFetch(providerRoutes)
    const { wrapper } = await mountForm()
  ...
  it('超出安全整数的 Token 窗口禁止提交', async () => {
    stubFetch(providerRoutes)
    const { wrapper } = await mountForm()
    await wrapper.find('input[placeholder="启用模型必填"]').setValue('9007199254740993')
    expect(wrapper.text()).toContain('正整数')
