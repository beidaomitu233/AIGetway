...
describe('AppLayout 四角色导航', () => {
  it.each([
    {
      role: 'SYSTEM_ADMIN',
      expects: ['运行摘要', '应用', '渠道', '上游 Key 池', '上游模型', '虚拟模型与路由', '限流策略', '熔断状态', '调用记录', '待发布变更', '配置发布', '旧访问凭证', '审计日志'],
      excludes: [],
    },
    {
      role: 'OPERATOR',
      expects: ['运行摘要', '应用', '渠道', '上游 Key 池', '上游模型', '虚拟模型与路由', '熔断状态', '调用记录', '旧访问凭证', '审计日志'],
      excludes: [],
    },
    {
      role: 'DEVELOPER',
      expects: ['运行摘要', '应用', '渠道', '上游模型', '虚拟模型与路由', '限流策略', '调用记录', '待发布变更', '接入说明与测试'],
      excludes: ['上游 Key 池', '审计日志'],
    },
    {
      role: 'VIEWER',
      expects: ['运行摘要', '应用', '渠道', '上游模型', '虚拟模型与路由', '限流策略', '调用记录', '待发布变更'],
      excludes: ['上游 Key 池', '旧访问凭证', '审计日志'],
    },
