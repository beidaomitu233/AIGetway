...
| P5-C-04 | Provider 在完成或失败后重复/迟到回调时，原实现可能重复结算、重复通知，或由旧 Attempt 的迟到 `onComplete` 改写恢复中的 Trace。 | StreamAccumulator 为每个 Attempt 增加终态 CAS，并串行化 `onNext`、`onError`、`onComplete` 与取消回调；首个终态之后的回调直接丢弃。 | `duplicateStreamTerminalCallbacksAreIgnoredPerAttempt` 回归通过：重复 `onComplete` 与迟到 `onError` 未产生第二次结算或终态通知，Trace 保持 `SUCCEEDED`。 | 已验证 |
...
BUILD SUCCESS；OpenAiCompatibleAdapterTest 6/6、JdbcTraceStoreTest 6/6、V1ControllerStreamTest 1/1、ChatPipelineTest 18/18、ChatPipelineRecoveryTest 9/9
...
- 终态幂等回归通过：每个 Attempt 的重复 `onComplete`、迟到 `onError` 和取消竞争均由一次性门控收敛，容量结算与监听器终态通知不重复。
