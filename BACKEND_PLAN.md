...
> 交付说明（2026-09-05）：BE-013—BE-018 已实现并通过 mvn test（全工程 121 例，本包新增 26 例）；BE-013/014/015/017/018 的真实 PostgreSQL 约束、行锁与同事务原子性证据依赖 DB-P02 迁移落地后补充联调复核；BE-015 的 available-models 与批量检测执行依赖 BE-P05 Adapter 注入（CheckInvoker/ModelListPort 端口已定义，以桩实现验证编排语义）；凭证检测的路由级容量预占与 BE-017 探测的 CAPACITY_LIMITED 依赖 BE-P04 容量组件。实现为契约优先口径，非真实联调完成。
