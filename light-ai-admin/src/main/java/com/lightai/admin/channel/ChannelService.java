...
import com.lightai.client.channel.HeaderPolicies;
...
public class ChannelService {
    ...
    public ChannelService(DataSource dataSource, JdbcChannelRepository providerRepository,
    ...
    public PageResult<ChannelListItem> list(RequestContext context, Map<String, String> params) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            List<ChannelRecord> records = providerRepository.list(connection, filter,
                    qualifySort(query.sort()), query.limit(), (int) query.offset());
            long total = providerRepository.count(connection, filter);
            List<ChannelListItem> items = composeListItems(connection, records);
    ...
    public ChannelDetail detail(RequestContext context, String rawId) {
        RequestPermissions.require(context, Permissions.PROVIDER_VIEW);
        UUID id = parseId(rawId);
        try (Connection connection = dataSource.getConnection()) {
            ChannelRecord record = providerRepository.findLiveById(connection, id)
    ...
    private List<ChannelListItem> composeListItems(Connection connection, List<ChannelRecord> records) {
        if (records.isEmpty()) {
            return List.of();
        }
        List<UUID> ids = records.stream().map(ChannelRecord::id).toList();
        ...
        Map<UUID, Long> modelCounts = referenceRepository.countUpstreamModelsByChannels(connection, ids);
        Map<UUID, Long> poolCounts = referenceRepository.countCredentialsByChannels(connection, ids);
        ...
        for (CheckRecordRow row : checkRecordRepository.findLatestByTargets(connection,
                CheckRecordRow.TARGET_CHANNEL, ids)) {
            latestChecks.put(row.targetId(), row);
        }

        List<ChannelListItem> items = new ArrayList<>(records.size());
    ...
    private ChannelDetail toDetail(Connection connection, ChannelRecord record) {
        var state = runtimeStateRepository.findByEntity(connection, ENTITY_TYPE, record.id()).orElse(null);
        List<ChannelCheckRecord> recent = checkRecordRepository
                .findLatestByTarget(connection, CheckRecordRow.TARGET_CHANNEL, record.id(), DETAIL_CHECK_RECORDS)
    ...
    private ChannelCheckRecord toCheckRecord(CheckRecordRow row) {
        return new ChannelCheckRecord(
    ...
    public ManagementOperationResult<ChannelDetail> create(RequestContext context,
                                                            ChannelSaveCommand command) {
    ...
    public ManagementOperationResult<ChannelDetail> update(RequestContext context, String rawId,
                                                            ChannelSaveCommand command) {
        RequestPermissions.require(context, Permissions.PROVIDER_MANAGE);
        UUID id = parseId(rawId);
        ...
        DraftWriteResult result = draftWriteService.execute(new DraftWriteCommand(
                ...
                connection -> providerRepository.lockLiveById(connection, id)
                        .map(ChannelRecord::version).orElse(null),
                connection -> {
                    ChannelRecord current = providerRepository.lockLiveById(connection, id)
    ...
    public ManagementOperationResult<ChannelDetail> setEnabled(RequestContext context, String rawId,
        ...
        String requestId = context.requestId();
        try (Connection connection = dataSource.getConnection()) {
            ChannelRecord current = providerRepository.findLiveById(connection, id)
                    .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND,
                            "Provider不存在或已删除"));
            // 停用需要确认影响；引用关系变化后票据失效
            if (!enabled) {
                impactService.verifyConfirmedImpact(confirmedImpactVersion,
                        impactService.analyzeChannel(connection, current.id(), current.name()));
            }
        } catch (LightAiException e) {
            throw e;
        } catch (Exception e) {
    ...
    public ManagementOperationResult<ChannelDetail> delete(RequestContext context, String rawId,
        ...
        try (Connection connection = dataSource.getConnection()) {
            ChannelRecord current = providerRepository.findLiveById(connection, id)
                    .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND,
                            "Provider不存在或已删除"));
            var fresh = impactService.analyzeChannel(connection, current.id(), current.name());
    ...
    public com.lightai.client.impact.ImpactAnalysis impact(RequestContext context, String rawId,
        ...
        try (Connection connection = dataSource.getConnection()) {
            ChannelRecord record = providerRepository.findLiveById(connection, id)
                    .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND,
                            "Provider不存在或已删除"));
            return impactService.analyzeChannel(connection, record.id(), record.name());
        } catch (LightAiException e) {
            throw e;
        } catch (Exception e) {
    ...
    private List<FieldChange> createChanges(ChannelRecord before, ChannelSaveCommand command) {
