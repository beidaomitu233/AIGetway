package com.lightai.admin.upstream;
 ...
 */
public class UpstreamModelService {

    public static final String ENTITY_TYPE = "UPSTREAM_MODEL";
    ...
    private final com.lightai.storage.check.JdbcChannelCheckRecordRepository modelCheckRecordRepository;
    ...
    public UpstreamModelService(DataSource dataSource, JdbcUpstreamModelRepository modelRepository,
                                JdbcChannelRepository providerRepository,
    ...
    public UpstreamModelService(DataSource dataSource, JdbcUpstreamModelRepository modelRepository,
                                ...
                                DraftWriteService draftWriteService, ImpactService impactService,
                                PageResultFactory pageResultFactory, String sourceMode,
                                com.lightai.storage.check.JdbcChannelCheckRecordRepository modelCheckRecordRepository,
    ...
    public PageResult<UpstreamModelDetail> listByProvider(RequestContext context, UUID channelId,
        ...
        try (Connection connection = dataSource.getConnection()) {
            requireProviderLive(connection, channelId);
            List<UpstreamModelRecord> records = modelRepository.listByProvider(connection, channelId,
                    params.get("keyword"), parseBoolean(params.get("support_stream")),
                    parseBoolean(params.get("enabled")), query.sort(), query.limit(),
                    (int) query.offset());
            long total = modelRepository.countByProvider(connection, channelId,
                    params.get("keyword"), parseBoolean(params.get("support_stream")),
                    parseBoolean(params.get("enabled")));
            List<UpstreamModelDetail> items = new ArrayList<>(records.size());
            for (UpstreamModelRecord record : records) {
    ...
    public PageResult<UpstreamModelDetail> listAll(RequestContext context, Map<String, String> params) {
        RequestPermissions.require(context, Permissions.MODEL_VIEW);
        ListQuerySupport.ListQuery query = ListQuerySupport.parse(
                params.get("page"), params.get("page_size"), params.get("sort"),
                SORTABLE, "updated_at desc");
        UUID channelId = null;
        if (params.get("channel_id") != null && !params.get("channel_id").isBlank()) {
            channelId = ChannelService.parseId(params.get("channel_id"));
        }
        try (Connection connection = dataSource.getConnection()) {
            List<UpstreamModelRecord> records = modelRepository.listByProvider(connection, channelId,
                    params.get("keyword"), parseBoolean(params.get("support_stream")),
    ...
    public UpstreamModelDetail detail(RequestContext context, String rawId) {
        RequestPermissions.require(context, Permissions.MODEL_VIEW);
        UUID id = ChannelService.parseId(rawId);
        try (Connection connection = dataSource.getConnection()) {
            UpstreamModelRecord record = modelRepository.findLiveById(connection, id)
    ...
    public ManagementOperationResult<UpstreamModelDetail> create(RequestContext context, UUID channelId,
                                                                 UpstreamModelSaveCommand command) {
        ...
        DraftWriteResult result = draftWriteService.execute(new DraftWriteCommand(
                ...
                connection -> {
                    ChannelRecord provider = requireProviderLive(connection, channelId);
                    UpstreamModelRecord record = new UpstreamModelRecord(id, channelId,
                            ...
                            command.priceUnit(), command.currency(), statusOf(command.enabled()),
    ...
    public ManagementOperationResult<UpstreamModelDetail> update(RequestContext context, String rawId,
                                                                 UpstreamModelSaveCommand command) {
        RequestPermissions.require(context, Permissions.MODEL_MANAGE);
        UUID id = ChannelService.parseId(rawId);
        ...
        DraftWriteResult result = draftWriteService.execute(new DraftWriteCommand(
                ...
                connection -> modelRepository.lockLiveById(connection, id)
                        .map(UpstreamModelRecord::version).orElse(null),
                connection -> {
    ...
    public ManagementOperationResult<UpstreamModelDetail> setEnabled(RequestContext context,
        ...
        String requestId = context.requestId();
        try (Connection connection = dataSource.getConnection()) {
            UpstreamModelRecord current = modelRepository.findLiveById(connection, id)
                    .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND, "模型不存在或已删除"));
            if (enabled) {
                // 启用要求能力完整且 context 严格大于 max_output（C-014）
                UpstreamModelDetail detail = toDetail(connection, current);
        ...
        DraftWriteResult result = draftWriteService.execute(new DraftWriteCommand(
                ...
                connection -> {
                    UpstreamModelRecord current = modelRepository.lockLiveById(connection, id)
                            .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND,
                                    "模型不存在或已删除"));
                    UpstreamModelRecord saved = modelRepository.update(connection,
    ...
    public ManagementOperationResult<UpstreamModelDetail> delete(RequestContext context, String rawId,
                                                                 Long version,
                                                                 String confirmedImpactVersion) {
        RequestPermissions.require(context, Permissions.MODEL_MANAGE);
        UUID id = ChannelService.parseId(rawId);
    ...
    private void validateCommand(UpstreamModelSaveCommand command, boolean requireCompleteIfEnabled) {
    ...
    private boolean completeForEnable(UpstreamModelSaveCommand command) {
    ...
    private void validateDefaultValues(UpstreamModelSaveCommand command) {
    ...
    private void validatePrices(UpstreamModelSaveCommand command) {
    ...
    private UpstreamModelRecord withEnabled(UpstreamModelRecord current, boolean enabled) {
        return new UpstreamModelRecord(current.id(), current.channelId(), current.modelId(),
                ...
                current.outputPrice(), current.priceUnit(), current.currency(), statusOf(enabled),
                current.importSource(), current.importAdapterVersion(), current.version(),
                current.createdAt(), current.updatedAt());
    }

    private UpstreamModelDetail toDetail(Connection connection, UpstreamModelRecord record) {
        var state = runtimeStateRepository.findByEntity(connection, "UPSTREAM_MODEL",
                record.id()).orElse(null);
        String channelName = providerRepository.findLiveById(connection, record.channelId())
                .map(ChannelRecord::name).orElse("");
        return new UpstreamModelDetail(
                record.id().toString(), record.channelId().toString(), channelName,
    ...
    private ChannelRecord requireProviderLive(Connection connection, UUID channelId) {
        return providerRepository.findLiveById(connection, channelId)
    ...
    public com.lightai.client.channel.ChannelCheckRecord check(RequestContext context, String rawId,
        ...
        try (Connection connection = dataSource.getConnection()) {
            model = modelRepository.findLiveById(connection, id)
                    .orElseThrow(() -> new LightAiException(ErrorCode.OBJECT_NOT_FOUND, "模型不存在或已删除"));
            provider = providerRepository.findLiveById(connection, model.channelId())
        ...
        com.lightai.storage.check.CheckRecordRow row = new com.lightai.storage.check.CheckRecordRow(
                UUID.randomUUID(), com.lightai.storage.check.CheckRecordRow.TARGET_UPSTREAM_MODEL, id,
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            modelRuntimeStateWriter.upsertProviderState(connection, model.channelId(),
    ...
    private static String statusOf(boolean enabled) {
        return enabled ? UpstreamModelRecord.STATUS_ACTIVE : UpstreamModelRecord.STATUS_DISABLED;
    }
