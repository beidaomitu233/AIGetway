...
public class UpstreamModelController {
    ...
    private final UpstreamModelService modelService;
    private final ModelImportService importService;

    public UpstreamModelController(UpstreamModelService modelService, ModelImportService importService) {
    ...
    public ResponseEntity<String> createTopLevel(@RequestBody String body, HttpServletRequest request) {
        ...
        if (pid == null || pid.isBlank()) {
            pid = request.getParameter("channel_id");
        }
        if (pid == null || pid.isBlank()) {
    ...
    public ResponseEntity<String> job(@PathVariable String id, HttpServletRequest request) {
        ...
        return json(ManagementResponses.ok(new JobPayload(
                ...
                items.stream().map(item -> new ItemPayload(item.id().toString(),
                        item.upstreamModelId().toString(), item.sequence(), item.status(),
    ...
    record ItemPayload(String id, String upstreamModelId, int sequence, String status,
