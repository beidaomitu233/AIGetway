...
import com.lightai.client.upstream.UpstreamModelImportCommand;
...
public class ModelImportService {
    ...
    public ModelImportService(DataSource dataSource, JdbcChannelRepository providerRepository,
    ...
    public ImportResult importModels(RequestContext context, UpstreamModelImportCommand command) {
        RequestPermissions.require(context, Permissions.MODEL_IMPORT);
        UUID channelId = ChannelService.parseId(command.channelId());
        if (!UpstreamModelImportCommand.SOURCE_PROVIDER_API.equals(command.source())
                && !UpstreamModelImportCommand.SOURCE_ADAPTER_PRESET.equals(command.source())) {
        ...
        ChannelRecord provider = requireProvider(channelId);
        ...
        for (String modelId : modelIds) {
            try {
                if (modelExists(channelId, modelId)) {
                ...
                draftWriteService.execute(new DraftWriteCommand(
                        ...
                        context.sourceIpMasked(), "CREATE", "upstream_model", channelId.toString(),
                        0, null, connection -> {
                            // 导入强制停用：启用前必须补齐能力（C-014）
                            UpstreamModelRecord record = new UpstreamModelRecord(
                                    id, channelId, modelId, defaultDisplayName(modelId),
                                    "CHAT_TEXT", null, null, null, null, null, null, null, null,
                                    null, null, null, null, null, null, null, null, null, List.of(),
                                    java.math.BigDecimal.ZERO, java.math.BigDecimal.ZERO, 1000000,
                                    "USD", UpstreamModelRecord.STATUS_DISABLED, command.source(), null, 1L,
                                    OffsetDateTime.now(), OffsetDateTime.now());
                            modelRepository.insert(connection, record);
                            return new DraftEntityChange("upstream_model", id, modelId, "CREATE", 1L,
    ...
    public List<String> availableModelIds(RequestContext context, UUID channelId,
        ...
        throw new LightAiException(ErrorCode.MODEL_LIST_NOT_SUPPORTED,
                "Adapter 未提供模型目录来源：" + provider.providerType());
    }

    // ---------- 批量检测 ----------

    public BatchJobRecord createBatchCheck(RequestContext context, UUID channelId,
                                           List<UUID> upstreamModelIds, UUID channelCredentialId,
                                           String mode, Integer timeoutMs) {
        RequestPermissions.require(context, Permissions.MODEL_MANAGE);
        if (upstreamModelIds == null || upstreamModelIds.isEmpty()
                || upstreamModelIds.size() > UpstreamModelImportCommand.MAX_BATCH) {
            throw new LightAiException(ErrorCode.FIELD_VALIDATION_FAILED, "upstream_model_ids 不合法",
    ...
    public BatchJobRecord job(RequestContext context, String rawId) {
        RequestPermissions.require(context, Permissions.MODEL_MANAGE);
        UUID jobId = ChannelService.parseId(rawId);
        try (Connection connection = dataSource.getConnection()) {
    ...
    private ChannelRecord requireProvider(UUID channelId) {
        try (Connection connection = dataSource.getConnection()) {
    ...
    private boolean modelExists(UUID channelId, String modelId) {
        try (Connection connection = dataSource.getConnection()) {
    ...
    private static String batchCommandJson(List<UUID> modelIds, UUID channelCredentialId, String mode,
                                           Integer timeoutMs) {
        StringBuilder json = new StringBuilder("{\"upstream_model_ids\":[");
        ...
        json.append("],\"channel_credential_id\":")
                .append(channelCredentialId == null ? "null" : "\"" + channelCredentialId + "\"")
