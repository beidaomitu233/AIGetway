...
| P4-BE-006-D：旧授权更新与发布装配迁移 | 代码审查与修复模型/root | 2026-09-15 | — | 旧 updateModels 写入口、发布装配和观测查询依赖 | 管理 API、发布/观测回归与数据库兼容性 | 领取中 | 2026-09-15 |
...
- P4-BE-006-D 已领取：继续迁移应用详情旧 `updateModels` 写入口、发布装配和观测查询依赖；保留历史审计、调用和成本快照，完成后再复验 P4 顶层。
