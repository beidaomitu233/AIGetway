...
| P4-BE-006-C：管理旧模型授权写路径与发布装配迁移 | 代码审查与修复模型/root | 2026-09-15 | 应用创建兼容字段、旧模型写接口收口 | 旧模型授权写入、草稿/发布装配和依赖检查 | 管理 API、前端创建流程与迁移回归 | 领取中 | 2026-09-15 |
...
- P4-BE-006-C 已领取：收口应用创建的旧 `virtual_model_ids` 写入、旧模型授权更新入口和草稿/发布装配依赖；先同步前端创建契约，再移除旧写路径，保留历史查询兼容和审计/调用/成本快照。
