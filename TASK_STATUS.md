...
| DB-P21 | DB-211～DB-215 | 数据库执行模型 zcode-db-0912b / feature/database-p21-zcode-db-0912b | storage-jdbc 双方言版本迁移与存储支撑、admin ResourceApiContractTest 批量检测断言随存储契约更新；DATABASE_PLAN.md、COMMUNICATION.md、TASK_STATUS.md；不修改前端与 admin/client/runtime/server 生产代码 | 阻塞 | 2026-09-12：c794043 实现（V5 virtual_model_routes_and_sync 双方言迁移：virtual_model 更名与 code 唯一、route_candidate 三元组唯一与 conditions/status、upstream_model locked_fields、channel_credential 名称唯一、batch_check 列对齐解除 BE-P21-006 503 拦截、model_sync_job/item + 仓储）与文档提交已交付；mvn -B verify 14 模块 443 通过/16 环境跳过/0 失败，git diff --check 通过；真实 PostgreSQL/MySQL（SchemaGuard IT 缺 LAI_IT_DB_URL/LAI_IT_MYSQL_URL）与 MySQL 5.7 未验证，DB-211～215 保持未勾选；DB-P21-007/008 待确认登记 COMMUNICATION；独立工作目录 .worktrees/database-p21-zcode-db-0912b |
