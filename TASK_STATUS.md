...
| P4-BE-006-B：应用详情模型读取迁移 | 代码审查与修复模型/root | 2026-09-15 | 应用详情模型摘要 | 详情与模型权限读取不依赖 virtual_model | 删除旧目录表后的详情/API 回归 | 已验证 | 2026-09-15 |
| P4-BE-006-C：应用创建契约收口与模型映射入口迁移 | 代码审查与修复模型/root | 2026-09-15 | 创建表单移除旧模型选择、模型映射移入应用详情 | 创建接口拒绝旧 virtual_model_ids 并停止写入旧权限表 | 前后端创建契约与回归测试 | 已验证 | 2026-09-15 |
| P4-BE-006-D：旧授权更新与发布装配迁移 | 代码审查与修复模型/root | 2026-09-15 | — | 旧 updateModels 写入口、发布装配和观测查询依赖 | 管理 API、发布/观测回归与数据库兼容性 | 修复中 | 2026-09-15 |
...
- P4-BE-006-B 已验证：应用 `/models` 子资源、应用详情模型摘要和候选目录优先读取应用映射；删除 H2 `route_candidate`、`virtual_model` 后 `ApplicationServiceTest` 仍通过。
- P4-BE-006-C 已验证：创建表单不再读取或提交旧模型候选，创建接口拒绝非空 `virtual_model_ids` 并停止写入 `application_model_permission`；模型映射统一在应用详情配置。前端创建回归、身份切换清理回归、后端应用/密钥/API 契约测试 32/32 通过。
- P4-BE-006-D 修复中：已移除管理 API 旧 `PUT /admin/applications/{id}/models` 路由和前端调用定义，详情映射接口仍为唯一配置入口；服务内部历史写方法、发布装配和观测查询依赖待继续迁移。
...
### P4-BE-006-A / P4-BE-006-B / P4-BE-006-C 领取记录（2026-09-15）
...
### P4-BE-006-D 领取记录（2026-09-15）

- 已由代码审查与修复模型/root 领取，范围限定为应用详情旧模型授权更新写入口、发布装配和观测历史依赖；P4-BE-006-C 的创建契约收口已验证。
- 其他协作者不得重复领取该子包；完成前保持“领取中/待复验”，不得仅凭静态修改标记为已验证。
